...
export function authentication(req,res,next){
 ...
 if((path==='/auth/login'||path==='/auth/logout')&&req.method==='POST'){
  if(!req.headers.origin||req.headers.origin!==`${secure?'https':'http'}://${req.headers.host}`){reply(res,403,{error:`Acesso bloqueado (403): A origem da requisição (${req.headers.origin || 'desconhecida'}) não corresponde ao servidor seguro esperado (${req.headers.host}). Isso pode ocorrer se você estiver acessando a aplicação via IP diferente ou ocorreu falha de CORS.`});return;}
